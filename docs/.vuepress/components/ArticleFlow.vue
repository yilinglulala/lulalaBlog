    ...
    <ul class='time-line'>
        ...
        <li class='timeline-stone is-completed' v-for="(art,art_index) in articleList" :key="'art_'+art_index">
            <div class='list-content'>
                ...
                <span class="timeLabel">Last Updated:&nbsp;&nbsp;</span>
                <span class='time'> {{art.lastUpdated}}</span>
            </div>
        </li>
    </ul>
</template>
...
<script>
    export default {
        props:{
            path:{default:"/FAQ/Console/"},
            listTitle:{default:"FAQ-求索 #"}
        },
        name: "ArticleFlow",
        computed: {
            articleList () {
                ...
                this.$site.pages.forEach((item)=>{
                    if(item.path.indexOf(this.path)>-1&&
                        !item.title.includes("列表") && item.title!=this.listTitle){
                        pages.push({
